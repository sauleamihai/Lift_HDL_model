`timescale 1ns/1ps
`ifndef __test_citire_registre
`define __test_citire_registre

import uvm_pkg::*;
`include "uvm_macros.svh"

`include "mediu_verificare.sv"
`include "secventa_apb_citire.sv"
`include "secventa_req_ack.sv"

// Test pentru CITIREA registrelor:
//   - Verifica ca scrierile la RW pot fi citite inapoi
//   - Verifica ca citirile la RO returneaza date valide (nu X)
//   - Adresele invalide returneaza 0xFF (default in lift_apb.v)
//   - Asertiunea p_prdata_known prinde orice PRDATA = X
class test_citire_registre extends uvm_test;

  `uvm_component_utils(test_citire_registre)

  mediu_verificare         mediu_de_verificare;
  secventa_apb_citire      apb_seq;
  ...
  function new(string name = "test_citire_registre", uvm_component parent = null);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    `uvm_info("TEST", "Reset eliberat. Initializam secventa de citire.", UVM_NONE)

    apb_seq     = secventa_apb_citire::type_id::create("apb_seq");
    ...
    `uvm_info("TEST", "Pornim secventele CITIRE si REQ/ACK in paralel", UVM_NONE)
  ...
  virtual function void report_phase(uvm_phase phase);
    ...
    $display("╔══════════════════════════════════════════╗");
    $display("║    RAPORT FINAL TEST_CITIRE_REGISTRE     ║");
    $display("╠══════════════════════════════════════════╣");
